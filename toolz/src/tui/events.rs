...
pub fn handle(app: &mut App, key: KeyEvent) {
    match &app.screen.clone() {
        ...
        Screen::Traces => handle_traces(app, key.code),
    }
}
    ...
    match code {
        ...
        KeyCode::Enter => match app.main_selected {
            ...
            4 => {
                ...
                app.logs_scroll = usize::MAX; // will clamp to tail on first render
            }
            _ => {}
        },
        _ => {}
...
fn handle_sys(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Enter => {
            app.sys_output
                .push("Run `toolz sys` in terminal to execute selected tasks.".to_string());
        }
        _ => {}
    }
}
...
fn handle_ai(app: &mut App, code: KeyCode) {
    match code {
        ...
        KeyCode::Enter => {
            if !app.ai_input.is_empty() && !app.ai_waiting {
                ...
                app.ai_messages.push((
                    "tip".to_string(),
                    "Run `toolz ai chat` in a terminal for full AI chat with streaming."
                        .to_string(),
                ));
        ...
        KeyCode::Down => {
            app.ai_scroll =
                (app.ai_scroll + 1).min(app.ai_messages.len().saturating_sub(1));
        }
        KeyCode::Char(c) => {
            app.ai_input.push(c);
        }
        ...
        KeyCode::Enter => {
            if app.db_focus == DbFocus::QueryInput && !app.db_query_input.is_empty() {
                app.db_results = vec![
                    ...
                    "Run `toolz db query <name> '<sql>'` in terminal for actual results."
                        .to_string(),
                ];
...
fn handle_traces(app: &mut App, code: KeyCode) {
    match code {
        KeyCode::Char('q') | KeyCode::Esc => app.go_back(),
        KeyCode::Up | KeyCode::Char('k') => {
        ...
        // G = jump to tail (newest entries)
        KeyCode::Char('G') => {
            app.logs_scroll = usize::MAX; // clamped to max on render
        }
        // g = jump to top (oldest entries)
        KeyCode::Char('g') => {
            app.logs_scroll = 0;
        }
        // f = cycle through level filters
        KeyCode::Char('f') => {
            app.logs_filter = app.logs_filter.next();
            app.logs_scroll = usize::MAX; // re-tail after filter change
        }
